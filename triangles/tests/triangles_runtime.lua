-- Minimal portable Ipe runtime for the standalone Triangles test suite.
...
local function object(kind, attributes, data, position, symbol, children, text_value)
  local value = {
    kind = kind,
  ...
  function value:text() return self.text_value end
...
function ipe.Text(attributes, text_value, position)
  return object("text", attributes, nil, position, nil, nil, text_value)
end
...
ipeui = {}
dofile(assert(TRIANGLES_PATH, "TRIANGLES_PATH must name triangles.lua"))
api = assert(_G.TRIANGLES)
...
function new_model(initial, configuration)
  ...
  function page:isLocked(layer)
    return configuration.locked_layers and configuration.locked_layers[layer] == true or false
  end
  function page:primarySelection()
    for index, entry in ipairs(entries) do if entry.selected == 1 then return index end end
    return nil
  end
  ...
  function model:warning(title, detail)
    ...
    self.warnings[#self.warnings + 1] = self.last_warning
  end
